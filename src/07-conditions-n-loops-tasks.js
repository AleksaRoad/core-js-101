...
function isInsideCircle(circle, point) {
  ...
  return Boolean(x + y < circle.radius ** 2);
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; str.length > i; i += 1) {
    ...
    const symbol = array.shift(array[i]);
    if (newStr.indexOf(symbol) === -1) {
      return symbol;
    }
  }
  return null;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const arrayNumbers = [a, b].sort((z, x) => z - x);
  const start = isStartIncluded ? '[' : '(';
  const end = isEndIncluded ? ']' : ')';
  return `${start + arrayNumbers[0]}, ${arrayNumbers[1]}${end}`;
}
